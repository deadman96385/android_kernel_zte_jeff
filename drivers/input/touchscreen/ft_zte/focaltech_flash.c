...
#define FTXXXX_INI_FILEPATH_CONFIG "/etc/firmware/"
/*******************************************************************************
...
static unsigned char CTPM_FW_HLT[] = {
#include "ZTE_N818S_HLT_FT6336U_V19_20170704_app.i"
};

static unsigned char CTPM_FW_LCE[] = {
#include "ZTE_N818S_LCE_FT6336U_V14_20170717_app.i"
};
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] != 0) {
			*ucPVendorID = 0;
			FTS_DBG("Vendor ID Mismatch, REG1 = 0x%x, REG2 = 0x%x, Definition:0x%x, i_ret=%d\n",
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		dev_err(&client->dev, "[FTS] Step 3: CTPM ID FAILED,ID1 = 0x%x,ID2 = 0x%x\n",
			reg_val[0], reg_val[1]);
		continue;
	}
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	...
	pr_info("[FTS][%s] erase app area reg_val[0] = %x reg_val[1] = %x\n", __func__,
		reg_val[0], reg_val[1]);
	...
	for (j = 0; j < packet_number; j++) {
		...
		if (bt_ecc != bt_ecc_check)
			pr_info("[FTS][%s] Host checksum error bt_ecc_check = %x\n", __func__, bt_ecc_check);
		fts_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		for (i = 0; i < 30; i++) {
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
			...
			pr_info("[FTS][%s] reg_val[0] = %x reg_val[1] = %x\n", __func__, reg_val[0], reg_val[1]);
			usleep_range(1000, 1100);
		}
	}
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
...
int  fts_8606_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == fts_updateinfo_curr.upgrade_id_1
		     && reg_val[1] == fts_updateinfo_curr.upgrade_id_2) || (reg_val[0] == 0x86 &&
				     reg_val[1] == 0xA6)) {
			FTS_DBG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	snprintf(filepath, sizeof(filepath), "%s%s", FTXXXX_INI_FILEPATH_CONFIG, firmware_name);
	if (pfile == NULL) {
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	if (pbt_buf == NULL) {
		pr_info("fts_ctpm_fw_upgrade_with_app_file no memory\n");
		return -EPERM;
	}
...
{
	char crtFwVendorId = 0x00;

	if (!client || !fwfilename)
		return -EPERM;
	if (!strcmp(fwfilename, "")) {
		pr_info("focaltech_fwupdate pfwfilename is null\n");
		return -EPERM;
	}
	if (fts_read_reg(client, 0xa8, &crtFwVendorId) >= 0) {
		pr_info("%s: read current vendor id is 0x%x\n", __func__, crtFwVendorId);
	} else {
		pr_info("%s: read current vendor id failed!!!\n", __func__);
		return -EPERM;
	}

	return fts_ctpm_fw_upgrade_with_app_file(client, fwfilename);
}

static unsigned char focaltech_ctpm_get_fw_ver(unsigned char *pbt_buf,
		unsigned int dw_length)
{
	if (dw_length > 2) {
		return pbt_buf[dw_length - 2];
	} else {
		return 0xff;
	}
}

int focaltech_get_fw_ver(struct i2c_client *client, char *fwfilename)
{
	...
	if (!client || !fwfilename)
		return -EPERM;
	if (!strcmp(fwfilename, "")) {
		pr_info("focaltech_get_fw_ver pfwfilename is null\n");
		return -EPERM;
	}
	fwsize = fts_GetFirmwareSize(fwfilename);
	if (fwsize <= 0) {
	...
	pbt_buf = kmalloc(fwsize + 1, GFP_ATOMIC);
	if (pbt_buf == NULL) {
		pr_info("focaltech_get_fw_ver no memory\n");
		return -EPERM;
	}

	if (fts_ReadFirmware(fwfilename, pbt_buf)) {
		pr_err("%s - ERROR: request_firmware failed\n", __func__);
		kfree(pbt_buf);
		return -EPERM;
	}

	ret = focaltech_ctpm_get_fw_ver(pbt_buf, fwsize);
	pr_info("focaltech_get_fw_ver:%x\n", ret);
	kfree(pbt_buf);
	return ret;
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	dev_dbg(&client->dev,
		"[FTS] old setting: i2c_addr = 0x%x, voltage = %d, factory_id = 0x%x\n",
		buf[0], buf[2], buf[4]);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	if ((fts_updateinfo_curr.CHIP_ID == 0x11) || (fts_updateinfo_curr.CHIP_ID == 0x12) ||
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, FW_LEN);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n", __func__);
	} else if ((fts_updateinfo_curr.CHIP_ID == 0x54)) {
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5x26_ctpm_fw_upgrade(client, pbt_buf, FW_LEN);
		if (i_ret != 0) {
			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		} else {
#ifdef AUTO_CLB
		...
		i_ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, FW_LEN);
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, FW_LEN);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n", __func__);
	}
	return i_ret;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if ((uc_tp_fm_ver == FTS_REG_FW_VER) ||	(uc_tp_fm_ver < uc_host_fm_ver)
	    || (uc_tp_fm_ver == 0) || (uc_tp_fm_ver == 0xff)) {
...
int fts_6x36gu_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucVendorID)
{
	u8 reg_val[4] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[10];
...
int fts_6x36gu_ctpm_fw_upgrade_in_bootloader(struct i2c_client *client)
{
	...
	fts_6x36gu_ctpm_fw_upgrade_ReadVendorID(client, &ucVendorID);
...
int fts_ctpm_find_firmware_from_vendorid(struct i2c_client *client)
{
	u8 ucVendorID = 0;

	fts_read_reg(client, 0xa8, &ucVendorID);

	pr_info("%s:ucVendorID 0x%02X\n", __func__, ucVendorID);

	if ((ucVendorID == 0x82) && sizeof(CTPM_FW_HLT)) {
		pr_info("HLT module!!!\n");
		CTPM_FW = CTPM_FW_HLT;
		FW_LEN = sizeof(CTPM_FW_HLT);
	} else if ((ucVendorID == 0x87) && sizeof(CTPM_FW_LCE)) {
		pr_info("LCE module!!!\n");
		CTPM_FW = CTPM_FW_LCE;
		FW_LEN = sizeof(CTPM_FW_LCE);
	} else if ((ucVendorID == 0x11) && sizeof(CTPM_FW_LCE)) {
		pr_info("old LCE module!!!\n");
		CTPM_FW = CTPM_FW_LCE;
		FW_LEN = sizeof(CTPM_FW_LCE);
	} else if (sizeof(CTPM_FW_DEFAULT)) {
		pr_info("Unconfig vendor firmware,so used default firmware!!!\n");
		CTPM_FW = CTPM_FW_DEFAULT;
		FW_LEN = sizeof(CTPM_FW_DEFAULT);
	} else {
		pr_err("[FTS] the vendor id: %d, not config the vendor firmware!.\n", ucVendorID);
		CTPM_FW = NULL;
		FW_LEN = 0;
		return -EIO;
	}
...
int fts_ctpm_auto_upgrade_select(struct i2c_client *client)
{
	u8 uc_tp_fm_chipid = 0;

	fts_read_reg(client, FTS_REG_ID, &uc_tp_fm_chipid);

	pr_info("%s:uc_tp_fm_chipid 0x%02X\n", __func__, uc_tp_fm_chipid);
	if ((uc_tp_fm_chipid == 0x64) || (uc_tp_fm_chipid == 0x54))  {
		fts_ctpm_find_firmware_from_vendorid(client);
